...
var GameLayer=(function(){
	function GameLayer(){
		...
		if(GameLayer._ins)throw('use function get ins()!!!');
		this.init();
	}
	...
	__getset(1,GameLayer,'ins',function(){
...
var GameMgr=(function(){
	function GameMgr(){
		if (GameMgr._ins)throw("use function get ins()!!!");
	}

	__class(GameMgr,'core.GameMgr');
	__getset(1,GameMgr,'ins',function(){
		return GameMgr._ins=GameMgr._ins|| new GameMgr;
	});

	GameMgr.start=function(){
		GameMgr.initModule();
		var complete=Handler.create(null,function(){
			EventCenter.send("OPEN_ENTER_ANI_VIEW");
		});
		GameMgr.loadRes(complete);
	}

	GameMgr.initModule=function(){
	...
	GameMgr.loadRes=function(complete){
		var path='res/atlas/resources.atlas';
...
var BlockModel=(function(){
	...
	/**获取最宽的一行 */
	__getset(0,__proto,'MaxCol',function(){
		var result=0;
		for (var i=0,iLen=this._mapArr.length;i < iLen;i++){
...
var BlockModule=(function(){
	...
	__proto.onOpenBLockView=function(index){
...
var EnterModule=(function(){
	function EnterModule(){
		this.enterView=null;
		this.enterAniView=null;
		if (EnterModule._ins)throw ("use function get ins()!!!");
	}

	__class(EnterModule,'module.enter.EnterModule');
	var __proto=EnterModule.prototype;
	__proto.addListener=function(){
		EventCenter.add("OPEN_ENTER_VIEW",this,this.onOpenEnterView);
		EventCenter.add("CLOSE_ENTER_VIEW",this,this.onCloseEnterView);
		EventCenter.add("OPEN_ENTER_ANI_VIEW",this,this.onOpenEnterAniView);
		EventCenter.add("CLOSE_ENTER_ANI_VIEW",this,this.onCloseEnterAniView);
	}

	__proto.onCloseEnterAniView=function(){
		if(this.enterAniView){
			GameLayer.ins.popLayer.removeChild(this.enterAniView);
			this.enterAniView=null;
		}
	}

	__proto.onOpenEnterAniView=function(){
		this.enterAniView=new EnterAniView;
		GameLayer.ins.popLayer.addChild(this.enterAniView);
	}

	__proto.onCloseEnterView=function(){
		if (this.enterView){
			GameLayer.ins.topLayer.removeChild(this.enterView);
			this.enterView=null;
		}
	}

	__proto.onOpenEnterView=function(){
		this.enterView=new EnterView;
		GameLayer.ins.topLayer.addChild(this.enterView);
	}

	__getset(1,EnterModule,'ins',function(){
		return EnterModule._ins=EnterModule._ins|| new EnterModule;
	});

	EnterModule._ins=null;
	return EnterModule;
})()


//class module.event.EventCenter
var EventCenter=(function(){
	function EventCenter(){}
	__class(EventCenter,'module.event.EventCenter');
	EventCenter.add=function(type,caller,callBack){
		var value={
			caller:caller,
			callBack:callBack
		};
		var arr=EventCenter.eventDic.get(type);
		if (arr){
			...
			arr=[value];
			EventCenter.eventDic.set(type,arr);
		}
	}

	EventCenter.remove=function(type,caller,callBack){
		var arr=EventCenter.eventDic.get(type);
		if (arr){
			for (var i=0;i < arr.length;i++){
				if (arr[i].caller==caller){
	...
	EventCenter.send=function(type,args){
		var arr=EventCenter.eventDic.get(type);
		if (arr){
			for (var i=0;i < arr.length;i++){
	...
	EventCenter.getDataByType=function(type){
		return EventCenter.eventDic.get(type);
	}

	__static(EventCenter,
	['eventDic',function(){return this.eventDic=new Dictionary;}
	]);
	return EventCenter;
})()


//class module.event.EventType
var EventType=(function(){
	function EventType(){}
	__class(EventType,'module.event.EventType');
	EventType.OPEN_BLOCK_VIEW="OPEN_BLOCK_VIEW";
	EventType.OPEN_ENTER_ANI_VIEW="OPEN_ENTER_ANI_VIEW";
...
//class module.levelselect.LevelSelectModel
var LevelSelectModel=(function(){
	...
	__proto.nextLevel=function(){
		if (this._openLevel==this.maxLevel)return;
	...
	__proto.init=function(){
		...
		for (var i=0;i < 12;i++){
			var obj={};
			obj.itemBg=this.closeSkin;
			obj.itemLb=i+1+'';
			this.leverSource.push(obj);
		}
		this.openLevel=1;
		this.leverSource[this._openLevel-1].itemBg=this.openSkin;
	}

	__proto.updateSkin=function(){
		for (var i=0,iLen=this.leverSource.length;i < iLen;i++){
			if (i+1 <=this.openLevel){
				this.leverSource[i].itemBg=this.openSkin;
				}else {
				this.leverSource[i].itemBg=this.closeSkin;
			}
		}
	}
	...
	__getset(1,LevelSelectModel,'ins',function(){
		return LevelSelectModel._ins=LevelSelectModel._ins|| new LevelSelectModel;
	});

	LevelSelectModel._ins=null;
	return LevelSelectModel;
})()


//class module.levelselect.LevelSelectModule
var LevelSelectModule=(function(){
	function LevelSelectModule(){
		this.levelSelectView=null;
		if (LevelSelectModule._ins)throw ("use function get ins()!!!");
	}

	__class(LevelSelectModule,'module.levelselect.LevelSelectModule');
	var __proto=LevelSelectModule.prototype;
	__proto.addListener=function(){
		EventCenter.add("OPEN_LEVEL_SELECT_VIEW",this,this.onOpenLevelSelectView);
		EventCenter.add("CLOSE_LEVEL_SELECT_VIEW",this,this.onCloseLevelSelectView);
	}

	__proto.onCloseLevelSelectView=function(){
		if (this.levelSelectView){
			GameLayer.ins.windowLayer.removeChild(this.levelSelectView);
			this.levelSelectView=null;
		}
	}

	__proto.onOpenLevelSelectView=function(){
		this.levelSelectView=new LevelSelectView;
		GameLayer.ins.windowLayer.addChild(this.levelSelectView);
	}

	__getset(1,LevelSelectModule,'ins',function(){
		return LevelSelectModule._ins=LevelSelectModule._ins|| new LevelSelectModule;
	});

	LevelSelectModule._ins=null;
	return LevelSelectModule;
})()
...
var WinModule=(function(){
	function WinModule(){
		this.winView=null;
		if (WinModule._ins)throw("use function get ins()!!!");
	}

	__class(WinModule,'module.win.WinModule');
	var __proto=WinModule.prototype;
	__proto.addListener=function(){
		EventCenter.add("OPEN_WIN_VIEW",this,this.onOpenWinDlg);
		EventCenter.add("CLOSE_WIN_VIEW",this,this.onCloseWinDlg);
	}

	__proto.onCloseWinDlg=function(){
		if (this.winView){
			GameLayer.ins.popLayer.removeChild(this.winView);
			this.winView=null;
		}
	}

	__proto.onOpenWinDlg=function(){
		this.winView=new WinView;
		GameLayer.ins.popLayer.addChild(this.winView);
	}

	__getset(1,WinModule,'ins',function(){
		return WinModule._ins=WinModule._ins|| new WinModule;
	});
...
//class utils.RandomUtils
var RandomUtils=(function(){
	function RandomUtils(){}
	__class(RandomUtils,'utils.RandomUtils');
	RandomUtils.createInt=function(int1,int2){
		(int2===void 0)&& (int2=0);
		...
		var dis=Math.abs(int2-int1);
		var result=Math.random()*dis+int1 >> 0;
		return result;
	}

	RandomUtils.fulfillArr=function(arr,fillValue){
		(fillValue===void 0)&& (fillValue=[0]);
		for (var i=0;i < arr.length;i++){
			if (((arr[i])instanceof Array)){
				RandomUtils.fulfillArr(arr[i],fillValue);
				}else {
				var n=RandomUtils.createInt(0,fillValue.length);
				arr[i]=fillValue[n];
			}
		}
	}
...
var BlockComp=(function(_super){
	...
	__proto.update=function(){
		...
		var row=arr.length;
		var col=BlockModel.ins.MaxCol;
		var wSize=this.width / col;
...
//class ui.EnterAniViewUI extends laya.ui.View
var EnterAniViewUI=(function(_super){
	function EnterAniViewUI(){
		this.imgAni=null;
		EnterAniViewUI.__super.call(this);
	}

	__class(EnterAniViewUI,'ui.EnterAniViewUI',_super);
	var __proto=EnterAniViewUI.prototype;
	__proto.createChildren=function(){
		laya.ui.Component.prototype.createChildren.call(this);
...
var EnterViewUI=(function(_super){
	function EnterViewUI(){
		this.btnLevelMode=null;
		this.btnEndlessMode=null;
		EnterViewUI.__super.call(this);
	}

	__class(EnterViewUI,'ui.EnterViewUI',_super);
	var __proto=EnterViewUI.prototype;
	__proto.createChildren=function(){
		laya.ui.Component.prototype.createChildren.call(this);
...
//class ui.LevelSelectViewUI extends laya.ui.View
var LevelSelectViewUI=(function(_super){
	function LevelSelectViewUI(){
		this.levelList=null;
		LevelSelectViewUI.__super.call(this);
	}

	__class(LevelSelectViewUI,'ui.LevelSelectViewUI',_super);
	var __proto=LevelSelectViewUI.prototype;
	__proto.createChildren=function(){
		laya.ui.Component.prototype.createChildren.call(this);
...
//class ui.WinViewUI extends laya.ui.View
var WinViewUI=(function(_super){
	function WinViewUI(){
		this.nextBtn=null;
		WinViewUI.__super.call(this);
	}

	__class(WinViewUI,'ui.WinViewUI',_super);
	var __proto=WinViewUI.prototype;
	__proto.createChildren=function(){
		laya.ui.Component.prototype.createChildren.call(this);
...
var BlockView=(function(_super){
	...
	__proto.movePowerSmall=function(){
		var _$this=this;
		var delay=600;
		Tween.to(this.powersmall1,{x:223,y:81},delay,null,Handler.create(this,addPowerNum));
		Tween.to(this.powersmall2,{x:189,y:149},delay,null,Handler.create(this,addPowerNum));
		Tween.to(this.powersmall3,{x:116,y:198},delay,null,Handler.create(this,addPowerNum));
		function addPowerNum (){
			_$this.powerNum++;
			if (_$this.powerNum >=3){
				_$this.powerNum=0;
				_$this.initPowerSmall();
			}
		}
	}

	__proto.initPowerSmall=function(){
		var _$this=this;
		Laya.timer.once(50,null,function(){
			_$this.powersmall1.pos(119,43);
			_$this.powersmall2.pos(101,75);
			_$this.powersmall3.pos(68,94);
			_$this.powersmall1.visible=false;
			_$this.powersmall2.visible=false;
			_$this.powersmall3.visible=false;
		});
		if (!this._isWin){
			Laya.timer.once(500,null,function(){
				_$this.powersmall1.visible=true;
				_$this.powersmall2.visible=true;
				_$this.powersmall3.visible=true;
				_$this.movePowerSmall();
			});
			}else {
			this.powersmall1.visible=true;
			this.powersmall2.visible=true;
			this.powersmall3.visible=true;
		}
	}
	...
	__proto.changeLightBulb=function(isWin,callBack){
		...
		var lightPCBSkin='resources/background_pcb_1.png';
		var darkPCBSkin='resources/background_pcb_2.png';
		if (isWin){
			this._isWin=true;
...
//class module.enter.EnterAniView extends ui.EnterAniViewUI
var EnterAniView=(function(_super){
	function EnterAniView(){
		EnterAniView.__super.call(this);
		this.alpha=0;
		Laya.timer.once(500,this,this.initView);
	}

	__class(EnterAniView,'module.enter.EnterAniView',_super);
	...
	__proto.initView=function(){
		...
		Tween.to(_this,{alpha:1},800,null,Handler.create(null,function(){
			...
			var curStep=0;
			function nextStep (){
				Laya.timer.once(stepDelay[curStep],null,function(){
					_$this.imgAni.skin='resources/enter_anim_'+stepSkin[curStep]+'.png';
					curStep++;
					if (curStep < stepLen){
						nextStep();
						}else {
...
var EnterView=(function(_super){
	function EnterView(){
		EnterView.__super.call(this);
		this.on("click",this,this.onclick);
	}

	__class(EnterView,'module.enter.EnterView',_super);
	var __proto=EnterView.prototype;
	__proto.onclick=function(evt){
		switch (evt.target){
			case this.btnLevelMode:
				EventCenter.send("CLOSE_ENTER_VIEW");
				EventCenter.send("OPEN_LEVEL_SELECT_VIEW");
				break ;
			case this.btnEndlessMode:
				EventCenter.send("CLOSE_ENTER_VIEW");
				EventCenter.send("OPEN_LEVEL_SELECT_VIEW");
				break ;
			default :
...
//class module.levelselect.LevelSelectView extends ui.LevelSelectViewUI
var LevelSelectView=(function(_super){
	function LevelSelectView(){
		LevelSelectView.__super.call(this);
	}
	...
	__proto.initialize=function(){
		...
		this.mouseEnabled=true;
		this.levelList.dataSource=LevelSelectModel.ins.leverSource;
	...
	__proto.onMouseHandler=function(e,index){
		...
		EventCenter.send("OPEN_BLOCK_VIEW",[index+1]);
	}
...
//class module.win.WinView extends ui.WinViewUI
var WinView=(function(_super){
	function WinView(){
		WinView.__super.call(this);
	}

	__class(WinView,'module.win.WinView',_super);
	var __proto=WinView.prototype;
	__proto.initialize=function(){
		this.nextBtn.on("click",this,this.onclick);
	}
